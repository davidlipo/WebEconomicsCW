...
#Get x and y for LR
testy = test[[ycolumn]]
testbidprice = testOriginal[['bidprice']]
...
for x in range(1, 10):
	...
	prediction = [baseBid * i[0] / avg for i in predictionOriginal]

	#Check the accuracy
	#print(accuracy_score(testy, prediction))
	total = [0]
	totalClick = [0]
	success = [j.click for i, j, k in zip(prediction, testy.itertuples(), testbidprice.itertuples()) if addToTotal(i, k.bidprice, j.click)]
	print(baseBid) #Base Bid
